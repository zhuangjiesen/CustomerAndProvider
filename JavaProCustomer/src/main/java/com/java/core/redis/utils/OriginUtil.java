...
public class OriginUtil {
    ...
    private static String KEY_PREFIX = "origin_";
    private static byte[] data = null;

    public static void originToRedis(Jedis jedis, int count, BizEvent sourceData) throws Exception {
        jedis.flushAll();
    ...
    public static void setToRedis(Jedis jedis, int count, BizEvent sourceData) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ObjectOutputStream objectOuts = new ObjectOutputStream(byteArrayOutputStream);
            objectOuts.writeObject(sourceData);
        ...
        System.out.println("origin redis set time : " + setTime);
    ...
    public static void getFromRedis(Jedis jedis, int count, BizEvent sourceData) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ObjectInputStream objectIns = new ObjectInputStream(byteArrayInputStream);
            BizEvent bizEventCache = (BizEvent) objectIns.readObject();
            byteArrayInputStream.close();
        }
        long getTime = (System.currentTimeMillis() - start);
        System.out.println("origin redis get time : " + getTime);
