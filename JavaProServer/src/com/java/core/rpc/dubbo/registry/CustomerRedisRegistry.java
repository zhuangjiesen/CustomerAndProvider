...
public class CustomerRedisRegistry extends FailbackRegistry {
    ...
    private final ConcurrentMap<String, CustomerRedisRegistry.Notifier> notifiers = new ConcurrentHashMap<String, CustomerRedisRegistry.Notifier>();
    ...
    public CustomerRedisRegistry(URL url) {
        ...
        boolean testOnBorrow = url.getParameter("test.on.borrow", true);
        boolean testOnReturn = url.getParameter("test.on.return", false);
        boolean testWhileIdle = url.getParameter("test.while.idle", false);
        ...
        if (url.getParameter("max.active", 0) > 0) {
            int maxActive = url.getParameter("max.active", 0);

        }

        if (url.getParameter("max.wait", url.getParameter("timeout", 0)) > 0) {
            int maxWait = url.getParameter("max.wait", url.getParameter("timeout", 0));
            jedisPoolConfig.setMaxWaitMillis(maxWait * 1000);
        }
        ...
        if (url.getParameter("time.between.eviction.runs.millis", 0) > 0) {
            int timeBetweenEvictionRunsMillis = url.getParameter("time.between.eviction.runs.millis", 0);
        ...
        for (String address : addresses) {
            ...
            JedisPool jedisPool = new JedisPool(jedisPoolConfig, host , port, url.getParameter(Constants.TIMEOUT_KEY, Constants.DEFAULT_TIMEOUT) , password);
                    ...
                    for (URL url : new HashSet<URL>(getRegistered())) {
                        if (url.getParameter(Constants.DYNAMIC_KEY, true)) {
                            ...
                            if (jedis.hset(key, url.toFullString(), String.valueOf(System.currentTimeMillis() + expirePeriod)) == 1) {
                                log.info(String.format("publish - key : %s " , key));
    ...
    public void doSubscribe(final URL url, final NotifyListener listener) {
        ...
        CustomerRedisRegistry.Notifier notifier = notifiers.get(service);
        if (notifier == null) {
            CustomerRedisRegistry.Notifier newNotifier = new CustomerRedisRegistry.Notifier(service);
    ...
    private class NotifySub extends JedisPubSub {
        ...
        public void onMessage(String key, String msg) {
            ...
            log.info(String.format("NotifySub key : %s , msg : %s " , key , msg));
                                ...
                                try {
                                    ...
                                    try {
                                        if (service.endsWith(Constants.ANY_VALUE)) {
                                            ...
                                            jedis.psubscribe(new CustomerRedisRegistry.NotifySub(jedisPool), service); // 阻塞
                                        } else {
                                            if (! first) {
                                                first = false;
                                            ...
                                            log.info(String.format("service : %s " , service));
                                            jedis.psubscribe(new CustomerRedisRegistry.NotifySub(jedisPool), service + Constants.PATH_SEPARATOR + Constants.ANY_VALUE); // 阻塞
                                        }
                                        break;
                                    } finally {
